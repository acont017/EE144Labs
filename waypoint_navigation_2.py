...
class Turtlebot():
    def __init__(self):
        ...
        self.T = 2  #Time to travel point to point.
    ...
    def move_to_point(self, point):
        ...
        # hint: you can have access to x, y by point[0], point[1]
            ...
            My = np.array([[0,0,0,0,0,1], [self.T**5,self.T**4,self.T**3,self.T**2,self.T,1],\
             [0,0,0,0,1,0], [5*self.T**4,4*self.T**3,3*self.T**2,2*self.T, 1, 0],[0,0,0,2,0,0],\
             [20*self.T**3,12*self.T**2,6*self.T,2,0,0]])
            x = np.array([self.pose.x, point[0], self.vel.linear.x, 0, 0, 0])
            y = np.array([self.pose.y, point[1], self.vel.linear.y, 0, 0, 0])
            ...
            for t in np.arange(0,self.T,0.1):
                ...
                if theta > pi:  # designed for orientation feedback (-pi, pi)
                    theta = theta - 2*pi
                elif theta < -pi:
                    theta = theta + 2*pi
